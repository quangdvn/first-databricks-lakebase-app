...
dbutils.widgets.text(
    "watchlist_table_name", "watchlist", "Source table (watchlist symbols)"
)
dbutils.widgets.text(
    "news_table_name", "ticker_news_documents", "Destination table (raw news)"
)
dbutils.widgets.text(
    "embeddings_table_name", "ticker_news_embeddings", "Destination table (vectors)"
)
dbutils.widgets.text(
    "chunk_embeddings_table_name",
    "ticker_news_chunk_embeddings",
    "Destination table (chunk vectors)",
)
dbutils.widgets.text(
...
dbutils.widgets.text(
    "max_requests_per_minute", "5", "Massive API rate limit (free tier is strict)"
)
...
print("  Using raw credentials from secret (no OAuth)")
...
def get_watchlist_tickers() -> list[str]:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            f"SELECT DISTINCT symbol FROM {WATCHLIST_TABLE_NAME} WHERE symbol IS NOT NULL"
        )
...
def fetch_news_for_ticker(
    session: requests.Session, ticker: str, limit: int
) -> list[dict]:
...
print("Run the next cell to insert them using executeLakebasePostgresSql tool.")
...
for i in range(0, len(news_df), batch_size):
    ...
    if (i + batch_size) % 128 == 0:
        print(
            f"  Processed {min(i + batch_size, len(news_df))}/{len(news_df)} documents"
        )
...
if len(embeddings_rows) > 0:
    print(
        f"Inserting {len(embeddings_rows)} embeddings into {EMBEDDINGS_TABLE_NAME}..."
    )
    ...
    try:
        ...
        execute_values(
            cursor, insert_sql, insert_data, template=template, page_size=100
        )
        ...
        print(
            f"  UPDATE {EMBEDDINGS_TABLE_NAME} SET embedding = embedding::vector WHERE embedding IS NOT NULL;"
        )
...
content_df = news_df[
    news_df["article_url"].notna() & (news_df["article_url"] != "")
].copy()
...
for i in range(0, len(chunks_df), batch_size):
    ...
    if (i + batch_size) % 128 == 0:
        print(
            f"  Processed {min(i + batch_size, len(chunks_df))}/{len(chunks_df)} chunks"
        )
...
print(
    f"Computed {len(chunk_embeddings_df)} chunk embeddings using {EMBEDDING_MODEL_NAME}"
)
...
if len(chunk_embeddings_rows) > 0:
    print(
        f"Inserting {len(chunk_embeddings_rows)} chunk embeddings into {CHUNK_EMBEDDINGS_TABLE_NAME}..."
    )
    ...
    try:
        ...
        print(
            f"  UPDATE {CHUNK_EMBEDDINGS_TABLE_NAME} SET embedding = embedding::vector WHERE embedding IS NOT NULL;"
        )
